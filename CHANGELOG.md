...
### Added
- GPU catalog sorting, filtering, and pagination on `/gpus`: a new `GpuCatalogFilter` carries `Search`, `Sort` (`Newest` / `PriceAsc` / `PriceDesc` / `RatingDesc`), `MinPrice`, `MaxPrice`, `MinVramGb`, `AvailableOnly`, `Page`, and `PageSize` (default 12, max 48), and `IGpuService.GetCatalogAsync` now returns `TotalCount` / `TotalPages` for a Bootstrap pagination strip with windowed page links. Filter form exposes price range, VRAM minimum, and an "available only" toggle; sort defaults to Newest. The free-text `Search` already matches GPU model, so a separate model selector isn't surfaced in the UI. `<img>` tags on catalog cards and the detail page now use `loading="lazy"` and `decoding="async"`
- Rental extension flow: `IRentalService.ExtendAsync(renterId, rentalId, additionalHours)` validates the rental is active, the new total duration is ≤ 168h, and the renter's balance covers the extra cost; wraps a transaction that debits the renter, credits the owner (90/10 split), logs `RentalCharge` and `RentalEarning` ledger rows tied to the rental, bumps `EndTime` / `DurationHours` / `TotalCost` / `PlatformFee` / `OwnerEarnings`, clears `ExpiryNotifiedAt` so the expiry notifier rearms, and notifies both parties. New `POST /rentals/extend/{rentalId}` endpoint and an Extend modal on Active Rentals with live cost preview (rate × hours) and balance display
- Reference-number search on Rental History: `RentalService.GetHistoryAsync` now matches the `Search` term against both `Gpu.Model` and `Rental.ReferenceNumber`, and the History filter form is rebuilt as a multi-field card with Status, GPU model dropdown (populated from the renter's own history), and `DateFrom` / `DateTo` start-time range filters
...
### Changed
- Prorated early termination refund: `RentalService.TerminateAsync` now computes whole-hours-used (`Math.Ceiling`, clamped to the original duration), recalculates `TotalCost` / `PlatformFee` / `OwnerEarnings` for the actual usage, refunds the renter the unused share via a `Refund` `CreditTransaction`, and reverses the corresponding portion of the owner's earnings via a negative-amount `Revoke` ledger row — all inside the existing transaction. Termination notifications now mention the refund amount when one was issued. The Active Rentals terminate-modal copy reflects the new refund behavior
